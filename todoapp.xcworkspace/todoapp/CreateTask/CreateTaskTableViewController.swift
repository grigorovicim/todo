...
class CreateTaskTableViewController: UITableViewController {
    ...
    private var items = [
        0 : [
            ...
            TodoReusableTableViewCellItem.init(title: "Project", info: "", leftImageName: "file", rightImageName: "right_arrow", options: projects),
        ],
    ...
    var project: Project? {
        set {
            ...
            if let previous = items[0]![1].selection as? Project, previous.name != project.name {
                let matchingProject = projects.first { (currentProject) -> Bool in
                    currentProject.name == project.name
                }
                
                if let task = self.task {
                    matchingProject?.tasks.append(task)
                    previous.tasks.remove(at: previous.tasks.firstIndex(of: task)!)
                }
            }
